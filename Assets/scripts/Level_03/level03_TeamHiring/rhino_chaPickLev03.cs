...
public class rhino_chaPickLev03 : MonoBehaviour
{
	//mouse and screen 
	...
	bottunDone_TeamSelLev03 setPosButtonScript;

	moneyToBuy money;

	GameObject rhinoInfo;

	void Start ()
	{
		rhinoInfo = GameObject.Find ("rhinoInfo");
		...
		setPosButtonScript = GameObject.Find ("bottunDone").GetComponent<bottunDone_TeamSelLev03>();
	...
	void OnMouseDrag()  		
	{		
		rhinoInfo.renderer.enabled = true;
	...
	void OnMouseUp () 
	{
		rhinoInfo.renderer.enabled = false;
		if (transform.position.x < pauseBar.transform.position.x+2)
		{
			...
			else if (pos2Check.pos2IsFree == true && rhinoIsOnShelf == false)
			{
			...
			else if (pos3Check.pos3IsFree == true && rhinoIsOnShelf == false)
			{
			...
			else if (pos4Check.pos4IsFree == true && rhinoIsOnShelf == false)
			{
