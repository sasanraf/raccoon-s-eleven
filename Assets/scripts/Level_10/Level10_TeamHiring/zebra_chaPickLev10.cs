...
public class zebra_chaPickLev10 : MonoBehaviour {
	...
	bottunDone_TeamSelLev10 setPosButtonScript;
	
	moneyToBuy money;
	
	GameObject handCuffs_zebra;

	bool zebraWasArrestedBefore = false;
	
	void Start ()
	{
	
		handCuffs_zebra = GameObject.Find("handCuffs_zebra");
		
		if (PlayerPrefs.GetInt("zebraArrested") == 1)
		{
			handCuffs_zebra.renderer.enabled = true;
		}
		
		PlayerPrefs.GetInt("zebraArrested");

		int totalScore = PlayerPrefs.GetInt("Player Score");
		if (PlayerPrefs.GetInt("zebraArrested") == 1 && totalScore < 20000)
		{
			Destroy (gameObject);
		}
		...
		setPosButtonScript = GameObject.Find ("bottunDone").GetComponent<bottunDone_TeamSelLev10>();
	...
	void OnMouseUp () 
	{
		...
		{
			if (PlayerPrefs.GetInt("zebraArrested") == 0 || 
			    (PlayerPrefs.GetInt("zebraArrested") == 1 && ((money.moneyLeft - 20000) >= 0)))
			{
			if (pos1Check.pos1IsFree == true && zebraIsOnShelf == false)
			{
				...
				if (PlayerPrefs.GetInt("zebraArrested") == 1)
				{
					money.audio.Play ();
					money.buy(20000);
					handCuffs_zebra.renderer.enabled = false;
					PlayerPrefs.SetInt("zebraArrested" ,0) ;
					zebraWasArrestedBefore = true;
				}
	...
	void CheckzebraBacktoPosition()
	{
		if (setPosButtonScript.chaPos1 == "zebra")
		{
			...
			if (zebraWasArrestedBefore == true)
				{
					money.audio.Play ();
					money.refund(20000);
					handCuffs_zebra.renderer.enabled = true;
					PlayerPrefs.SetInt("zebraArrested" ,1);
					zebraWasArrestedBefore = false;
				}
